...
function WireGlobe() {
  ...
  return <div className="globe" aria-hidden="true"><svg viewBox="0 0 1200 560" preserveAspectRatio="none">
    <defs><linearGradient id="globeFade" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#fff" stopOpacity=".28"/><stop offset="1" stopColor="#fff" stopOpacity=".045"/></linearGradient><linearGradient id="pathBlue" x1="0" y1="0" x2="1" y2="0"><stop stopColor="#f14c13" stopOpacity="0"/><stop offset=".35" stopColor="#f14c13"/><stop offset="1" stopColor="#ff6a2f"/></linearGradient></defs>
...
function GithubActivity() {
  ...
  return <div className="github-activity">
    ...
    {data && (
      <>
        <p className="github-total">{data.total.toLocaleString("en-US")} commits across shipped systems — live since Mar 2026</p>
...
function App() {
  ...
  useEffect(() => {
    if (!palette) { setPaletteIndex(-1); return; }
    const onKey = (event) => {
      if (event.key === "ArrowDown") { event.preventDefault(); setPaletteIndex(i => (i + 1) % navItems.length); }
      else if (event.key === "ArrowUp") { event.preventDefault(); setPaletteIndex(i => (i - 1 + navItems.length) % navItems.length); }
      else if (event.key === "Enter" && paletteIndex >= 0) { event.preventDefault(); goTo(navItems[paletteIndex][2]); }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  ...
  return <main>
    ...
    {palette && <div className="command-backdrop" onMouseDown={() => setPalette(false)}><div className="command-palette" role="dialog" aria-modal="true" aria-label="Quick navigation" onMouseDown={event => event.stopPropagation()}><div className="command-head"><span>GO TO</span><div><kbd>ESC</kbd><button className="command-close" onClick={() => setPalette(false)} aria-label="Close menu">×</button></div></div>{navItems.map(([n, name, id], i) => <button key={id} ref={el => { if (i === paletteIndex && el) el.focus(); }} onClick={() => goTo(id)}><small>{n}</small><span>{name}</span><i>↵</i></button>)}</div></div>}
