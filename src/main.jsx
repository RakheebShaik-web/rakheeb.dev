...
function App() {
  ...
  return <main>
    <header className="nav"><a className="wordmark" href="#top" aria-label="Rakheeb, home">RAKHEEB</a><nav aria-label="Main navigation">{navItems.slice(1,5).map(([n, name, id]) => <a key={name} className={active === id ? "active" : ""} href={`#${id}`}><small>{n}</small>{name}</a>)}</nav><div className="nav-utilities"><button className="menu-trigger" onClick={() => setPalette(true)} aria-label="Open menu"><span className="menu-lines"><i/><i/></span><span>MENU</span></button><button onClick={() => setTheme(theme === "light" ? "dark" : "light")} aria-label={`Switch to ${theme === "light" ? "dark" : "light"} theme`}><span className="theme-mark">◐</span></button></div></header>
    ...
    <section className="hero" id="top"><div className="hero-meta"><span>HYDERABAD / INDIA</span><span>17.3850° N<br/>78.4867° E</span></div><div className="hero-copy"><p className="eyebrow">ALGORITHMIC TRADER · QUANT DEVELOPER</p><h1>Systematic edge<br/><em>engineered.</em></h1><p className="intro">Quantitative trading systems. Built with rigor. Executed with discipline.</p></div><p className="hero-note">A SYSTEM IS A HYPOTHESIS<br/>WITH CONSEQUENCES.</p></section>
    <section className="about section" id="about"><SectionLabel index="01">PROFILE / MANDATE</SectionLabel><Reveal className="about-copy"><p>I’m Rakheeb, an algorithmic trader at a hedge fund, working across quantitative strategies in U.S. equities and Indian options.</p><p>I research market ideas and develop automated trading systems that handle screening, risk, execution, and monitoring. My background in Computer Science and AI gives me the technical foundation to turn trading ideas into reliable systems.</p><p className="offdesk">I watch crypto charts, invest in mutual funds, and answer to a Persian cat who believes every keyboard belongs to him.</p></Reveal></section>
    ...
    <section className="contact section" id="contact"><SectionLabel index="05">OPEN CHANNEL</SectionLabel><h2>Let’s talk<br/><em>markets.</em></h2><div className="contact-actions"><a href="mailto:shaikrakheeb280@gmail.com"><span className="social-name"><SocialIcon name="email"/>Email</span><span>↗</span></a><a href="https://www.linkedin.com/in/rakheeb-shaik-aba0762b5/" target="_blank" rel="noreferrer"><span className="social-name"><SocialIcon name="linkedin"/>LinkedIn</span><span>↗</span></a><a href="https://github.com/RakheebShaik-web" target="_blank" rel="noreferrer"><span className="social-name"><SocialIcon name="github"/>GitHub</span><span>↗</span></a></div><footer><span>© 2026 RAKHEEB</span><span>BUILT WITH DISCIPLINE</span><a href="#top">BACK TO TOP ↑</a></footer></section>
  </main>;
}
