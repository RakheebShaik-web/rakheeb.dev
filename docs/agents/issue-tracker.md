...
- **Child ticket**: link it as a GitHub sub-issue. If sub-issues are unavailable, add it to a task list in the map and put `Part of #<map>` at the top of the child. Use a `wayfinder:<type>` label.
- **Blocking**: prefer GitHub's native issue dependencies. Fall back to a `Blocked by: #<n>` line when dependencies are unavailable.
